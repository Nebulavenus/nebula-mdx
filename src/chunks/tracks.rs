...
impl<'a, T: 'a> ctx::TryFromCtx<'a, Endian> for Track<T>
    where T: Default + ctx::TryFromCtx<'a, Endian, Error = scroll::Error>,
{
    ...
    fn try_from_ctx(src: &'a [u8], ctx: Endian) -> Result<(Self, usize), Self::Error> {
        ...
        value = src.gread_with(offset, ctx)?;
...
impl<T> ctx::TryIntoCtx<Endian> for Track<T>
    where T: Copy + Default + ctx::TryIntoCtx<Endian, Error = scroll::Error>,
{
    type Error = scroll::Error;
